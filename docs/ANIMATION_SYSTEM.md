...
Content is visible in server-rendered HTML before hydration and when JavaScript is unavailable. The observer unobserves completed elements and disconnects on unmount. No scroll event handlers are loaded. Newsletter feedback uses Motion with LazyMotion, a short opacity transition and button press feedback, respecting reduced motion through MotionConfig. The loading spinner is disabled with reduced motion.
